# bfparse.py by Nishan Maharjan (nmaharjan@csus.edu) for CSC 135 Spring 2025
...
def parseS(toks):
    ...
    tok = toks.next()
    if tok in ('>', '<','+','-',',','.'): 
        newnode.add_child(parseC(toks)) #parseC(toks)
        newnode.add_child(parseS(toks)) #parseS(toks)
    elif tok == '[': 
        newnode.add_child(parseL(toks)) #parseL(toks)
        newnode.add_child(parseS(toks)) #parseS(toks)
    elif tok in (None, ']'):
